...
import type { PreCopaPrediction } from "@/app/types";`n`nconst PRE_COPA_LOCKED = true;`n`nconst PRE_COPA_LOCKED = true;
...
export default function PreCopaPage() {
  ...
  useEffect(() => {
    if (!user) return;`n    if (PRE_COPA_LOCKED) {`n      setToast({ type: "error", message: "Os palpites Pr�-Copa foram encerrados." });`n      return;`n    }`n    if (PRE_COPA_LOCKED) {`n      setToast({ type: "error", message: "Os palpites Pr�-Copa foram encerrados." });`n      return;`n    }
