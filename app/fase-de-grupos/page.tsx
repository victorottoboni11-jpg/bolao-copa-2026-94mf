...
export default function FaseDeGruposPage() {
  ...
  useEffect(() => {
    ...
    const loadData = async () => {
      ...
      try {
        ...
        const predictionMap = await getPredictionsForMatches(user.id, groupMatches.map((match) => match.id));
        setPredictions(predictionMap);
      } catch (error) {
        console.error("Erro ao carregar jogos da fase de grupos:", error);
      } finally {
  ...
  const handleSavePrediction = async (matchId: string, homeScore: number, awayScore: number) => {
    ...
    try {
      ...
      console.error("Erro ao salvar palpite na fase de grupos:", error);
      setToast({ type: "error", message: "Erro ao salvar palpite" });
    } finally {
      ...
      <div className="mx-auto max-w-6xl space-y-8">
        <header className="rounded-2xl border border-[#00ffb2]/20 bg-gradient-to-br from-[#081116] to-[#070b16] p-6 shadow-lg">
          <div className="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
            <div>
              <p className="text-xs uppercase tracking-widest text-[#00ffb2]">Fase de Grupos</p>
              <h1 className="mt-2 text-3xl font-bold text-white">Jogos da fase de grupos</h1>
              <p className="mt-1 text-sm text-gray-400">Confira e atualize seus palpites para todos os jogos da fase de grupos.</p>
            </div>
          ...
          <div className="mt-4 text-sm text-slate-400">
            <span className="mr-2">Home</span>/ <span className="text-[#00ffb2]">Fase de Grupos</span>
          </div>
        </header>
        ...
        <div className="rounded-3xl border border-[#00ffb2]/20 bg-slate-950/90 p-6 shadow-[0_20px_60px_rgba(0,0,0,0.18)]">
          <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
            <div>
              ...
              <p className="mt-2 text-lg font-semibold text-white">{sortedMatches.length} partidas de grupo</p>
            </div>
